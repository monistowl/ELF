%This is a modified HRVAS.m to allow editing a signal within matlab, rather
%than just from an external .ibi  file. The rest of HRVAS is required for
%it to work. Forked by Tom Wilson, 2019.
...
%   -add error handling
    ...
    figH=700;   %height of GUI (pixels)
    figW=1300;  %width of GUI (pixels)
    ...
    %Main Figure
    h.MainFigure = figure('Name','PhysioData.hrvGui','NumberTitle','off', ...
        'HandleVisibility','on', ...
        'Units','normalized', ...
        'Position',[0 0 1 1 ],...
...
%         'Position',[.947 .08 .05 .8],'Callback', @btnRun_Callback);
    ...
    %Preview Button
     h.btnRecalculate=uicontrol(h.panelPre,'Style','pushbutton', ...
        'String','Recalculate...', 'Units', 'normalized', ...
        'Position',[.72 .93 .25 .06],'FontSize',8,...
        'Callback', @btnRecalculate_Callback);
...
%% GUI: Results - TimeFreq Tab
    ...
    set(h.btngrpTFPlot2,'SelectedObject',h.radioTFPlotPSD);
...
%     clear tmp
    ...
    settings = P.HRVAS_Analysis.settings; 
    %set controls to previous options
    setSettings(settings) 
    
    %if args, do w/o file input
    
    HRV=getHRV(P.ibis_a,settings);
    
    %open time freq wavelet surface PSD by default
    h.tabgroup.SelectedTab = h.tab6;
    h.btngrpTFPlot.SelectedObject = h.radioTFPlotWav;
    h.btngrpTFPlot2.SelectedObject = h.radioTFPlotSpec;
...
%                 exportHRV(outfile,{'asdf'},HRV,settings)
%             end
%         end    
    end
...
%     end
    ...
    function btnRecalculate_Callback(hObject, eventdata)
...
%% GUI: CreateTFTbl
    ...
    function output=getHRV(ibi,opt)
